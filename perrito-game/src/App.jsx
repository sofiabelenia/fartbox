...
// Configuración de niveles: Cantidad de cajas por nivel
// Cada nivel tiene menos cajas = más difícil (mayor probabilidad de elegir veneno)
const LEVEL_CONFIG = {
  1: 5,  // 4 comida + 1 veneno (20% riesgo)
  2: 4,  // 3 comida + 1 veneno (25% riesgo)
  3: 3,  // 2 comida + 1 veneno (33% riesgo)
  4: 2,  // 1 comida + 1 veneno (50% riesgo!)
  5: 2   // 1 comida + 1 veneno (50% riesgo!)
};

export default function App() {
  const [gameState, setGameState] = useState('menu'); // menu, playing, processing, gameover, victory
  const [level, setLevel] = useState(1);
  const [boxes, setBoxes] = useState(Array(9).fill(null)); // Array de 9 posiciones (3x3)
  const [dogStatus, setDogStatus] = useState('happy'); // happy, eating, sick, neutral
  const [score, setScore] = useState(0);
  const [message, setMessage] = useState('');
  const [showSipo, setShowSipo] = useState(false); // Estado para la animación "SIPO"
  const [showFome, setShowFome] = useState(false); // Estado para la animación "FOME"
  const [currentAudio, setCurrentAudio] = useState([]); // Audios activos
  const [usedPositions, setUsedPositions] = useState([]); // Posiciones ya usadas
  ...
  const startLevel = (currentLevel) => {
    ...
    const availablePositions = Array(9).fill(null)
      .map((_, idx) => idx)
      .filter(idx => !usedPositions.includes(idx));

    // Seleccionar posiciones aleatorias para este nivel
    const shuffled = [...availablePositions].sort(() => Math.random() - 0.5);
    const selectedPositions = shuffled.slice(0, numBoxes);

    // Crear array de 9 posiciones
    const newBoxes = Array(9).fill(null);

    // Colocar cajas en posiciones seleccionadas
    const poisonIndex = Math.floor(Math.random() * numBoxes);
    selectedPositions.forEach((position, idx) => {
      newBoxes[position] = {
        id: position,
        isPoison: idx === poisonIndex,
        isOpen: false,
        content: idx === poisonIndex ? 'poison' : 'food'
      };
    });

    setBoxes(newBoxes);
    setDogStatus('neutral');
    setMessage(`¡Encuentra toda la comida! Evita el veneno.`);
  ...
  const startGame = () => {
    ...
    setDogStatus('happy');
    setShowFome(false);
    setUsedPositions([]); // Resetear posiciones usadas
    startLevel(1);
  };
  ...
  const handleBoxClick = (index) => {
    ...
    if (selectedBox.isPoison) {
      // Perdió
      setDogStatus('sick');
      setMessage('¡Oh no! ¡Estaba podrido!');

      // Revelar todas las cajas para mostrar dónde estaba la comida
      setTimeout(() => {
        const revealedBoxes = boxes.map(b => b ? { ...b, isOpen: true } : null);
        ...
        // Mostrar animación FOME
        setShowFome(true);
      }, 1500);

    } else {
      // Encontró comida buena
      setDogStatus('eating');
      setMessage('¡Ñam ñam! ¡Qué rico!');

      // Reproducir sonido SIPO
      playSipoSound();

      // Activar animación SIPO
      setShowSipo(true);
      ...
      // Verificar si encontró todas las cajas con comida (todas excepto el veneno)
      const totalFood = boxes.filter(b => b && b.content === 'food').length;
      const openFood = newBoxes.filter(b => b && b.content === 'food' && b.isOpen).length;

      if (openFood === totalFood) {
        // Encontró toda la comida! Pasar al siguiente nivel
        // Reproducir sonido de nivel completado
        playSipoGanadorSound();
        ...
        // Aún faltan cajas por abrir, continuar jugando
        setTimeout(() => {
          setGameState('playing');
        }, 1500);
      }
        ...
        <div className="flex-1 p-6 bg-slate-50 flex items-center justify-center">
          ...
          {gameState === 'menu' && (
            <div className="text-center space-y-4">
              ...
              <p className="text-slate-600 mb-6">
                ¡Encuentra TODA la comida rica!<br/>
                ¡Cuidado! Una caja tiene veneno.<br/>
                <span className="text-sm font-semibold text-rose-500">Debes abrir todas las cajas con comida para ganar.</span>
              </p>
              <button
                onClick={startGame}
          ...
          {(gameState === 'playing' || gameState === 'processing') && (
            <div className="grid grid-cols-3 gap-4 w-full max-w-sm mx-auto">
              {boxes.map((box, idx) => {
                if (!box) return <div key={`empty-${idx}`} className="aspect-square" />;
                ...
                return (
                  <button
                    ...
                    className={`relative aspect-square rounded-xl shadow-md transition-all duration-300 transform bg-amber-200 border-b-4 border-amber-400
                      ${!box.isOpen ? 'hover:-translate-y-1 hover:shadow-lg cursor-pointer' : 'cursor-default'}
                    `}
          ...
          {gameState === 'gameover' && (
            <div className="text-center relative">
              ...
              <h2 className="text-3xl font-bold text-red-500 mb-2">🤮</h2>
              <p className="text-slate-600 mb-6">El perrito comió algo malo.</p>
              <div className="bg-white p-4 rounded-xl shadow-inner mb-6">
                <p className="text-sm text-slate-400 uppercase tracking-wide">Puntuación Final</p>
          ...
          {gameState === 'victory' && (
            <div className="text-center">
              <h2 className="text-3xl font-bold text-green-500 mb-2">¡Panza Llena! 🎉</h2>
              <p className="text-slate-600 mb-6">¡Completaste todos los niveles!</p>
              <div className="bg-yellow-50 p-6 rounded-xl border-2 border-yellow-200 shadow-sm mb-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-2 opacity-10">
                  <Trophy size={100} />
                </div>
                <p className="text-sm text-yellow-600 uppercase tracking-wide font-bold">Puntuación Total</p>
